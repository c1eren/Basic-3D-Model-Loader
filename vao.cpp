...
// If sending UVs as vec3
void Vao::setLayoutTex3(bool positions, bool normal, bool texture, bool color, bool block)
{
	if (!block)
	{
		unsigned int stride = 0;
		...
		{
			glEnableVertexAttribArray(2);
			glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, stride, (void*)(step));
			step += 12;
		}
		if (color)
		{
	...
	else
	{
		...
		{
			glEnableVertexAttribArray(2);
			glVertexAttribPointer(2, 9, GL_FLOAT, GL_FALSE, 0, (void*)(step));
			step += 36;
		}
		if (color)
		{
